/* ==========================================================================
   Vecode — agent.js · agent harness (v3 rebuild)
   Streaming LLM + 5 tools + fallback blocks + review pass. Rebuilt ground-up
   with clearer state machine, safer path handling, and deterministic fallback.
   ========================================================================== */
(function () {
  "use strict";
  ...
  const TOOL_DEFS = [
    { type: "function", function: { name: "write_file", description: "Create or overwrite a file in the project. path is relative, e.g. 'index.html', 'styles.css', 'script.js', 'pages/about.html'. Content is the full file text.", parameters: { type: "object", properties: { path: { type: "string", description: "Relative file path" }, content: { type: "string", description: "Full file content" } }, required: ["path","content"] } } },
    { type: "function", function: { name: "read_file", description: "Read a file from the project to see its current content.", parameters: { type: "object", properties: { path: { type: "string" } }, required: ["path"] } } },
    { type: "function", function: { name: "list_files", description: "List all files in the project with their sizes.", parameters: { type: "object", properties: {} } } },
    ...
    { type: "function", function: { name: "finish", description: "Call when the site is complete. Provide a message describing what was built, files created/modified, and next steps.", parameters: { type: "object", properties: { message: { type: "string", description: "Summary message for the user" } }, required: ["message"] } } }
  ];
...
5. If tools unexpectedly fail, output complete files as fenced blocks tagged with their relative paths, for example:
  ...
  function buildSystemPrompt(useTools) {
    ...
    const pluginOptions = {}; for (const id of enabledIds) pluginOptions[id] = S.getPlugin(id).options || {};
...
FINAL CHECK — before calling finish(), self-review the build against the anti-slop table above${pluginCtx.review.length ? " plus these plugin checks:\n" + pluginCtx.review.map((r) => " · " + r).join("\n") : ""}. Fix anything that fails.`;
  ...
  function buildReviewSystemPrompt(useTools) {
    ...
    const protocol = useTools ? "Use read_file, write_file, list_files, delete_file and finish." : `This provider has no file tools. You MUST write ALL complete text files as \`\`\`veccode:path fenced blocks when applying fixes; do not output partial patches or omit unchanged text files. Use four backticks when file content contains three backticks.`;
...
Rules:
 ...
 · After applying fixes, ${useTools ? "call finish() with" : "add"} a short summary of what you changed and why.`;
  ...
  function parseFileBlocks(text) {
    const files = {}; FENCE_RE.lastIndex = 0; let m;
  ...
  function stripFileBlocks(text) { FENCE_RE.lastIndex = 0; return text.replace(FENCE_RE, "").replace(/\n{3,}/g, "\n\n").trim(); }
  ...
  function summarizeAgentAction(mode, writtenFiles, deletedFiles, userPrompt) {
    ...
    if (files.length) {
      ...
      if (deletedFiles && deletedFiles.length) msg += `\n\n**Removed:** ${deletedFiles.map((f) => `\`${f}\``).join(", ")}`;
    ...
    if (S && S.listFiles && S.listFiles().length) return `I've checked the project files. Everything is up to date in the live preview. Tell me what you'd like to build, add, or customize next!`;
    ...
    try {
      switch (name) {
        case "write_file": {
          ...
          const content = typeof args.content === "string" ? args.content : args.content == null ? "" : JSON.stringify(args.content);
        ...
        case "finish": return "OK — the build is complete.";
  ...
  const harness = {
    running: false, _abort: null, usage: { input: 0, output: 0 }, events: {},
    on(ev, fn) { this.events[ev] = fn; },
    emit(ev, payload) { const fn = this.events[ev]; if (fn) { try { fn(payload); } catch (e) { console.error(e); } } },
    stop() { if (this._abort) this._abort.abort(); this.running = false; this.emit("stopped"); },
    _freshAbort() { this._abort = new AbortController(); return this._abort.signal; },
    _trackUsage(u) {
      if (!u) return;
    ...
    async run(userText, mode) {
      ...
      try {
        ...
        if (mode === "review") {
          ...
          const history = S.state.messages.slice(0, -1).slice(-8).map((m) => ({ role: m.role === "user" ? "user" : "assistant", content: m.content || "" }));
        ...
        let finalText = ""; let finishMessage = null;
        const writtenFiles = new Set(); const deletedFiles = new Set();
        for (let iter = 0; iter < MAX_TOOL_ITERATIONS; iter++) {
          if (signal.aborted) break;
          this.emit("iteration", iter + 1);
          const result = await window.Vecode.Providers.stream({ cfg, messages, tools: useTools ? TOOL_DEFS : [], temperature: S.settings.temperature },
            { onText: (t) => { finalText += t; this.emit("delta", { mode, text: t }); }, onToolStart: (tc) => this.emit("toolStart", tc), onToolDelta: (tc) => this.emit("toolDelta", tc) }, signal);
          ...
          if (blockPaths.length) {
            for (const p of blockPaths) { S.writeFile(p, blocks[p]); writtenFiles.add(p); }
            this.emit("filesWritten", blockPaths.map((p) => ({ path: p, bytes: blocks[p].length, via: "block" })));
            result.text = stripFileBlocks(result.text);
          }
          if (result.toolCalls && result.toolCalls.length) {
            const assistantMsg = { role: "assistant", content: result.text || "", tool_calls: result.toolCalls.map((tc) => ({ id: tc.id, name: tc.name, args: tc.args })) };
            ...
            for (const tc of result.toolCalls) {
              ...
              let out;
              if (tc.args && tc.args._raw !== undefined) out = "Error: could not parse the tool arguments as JSON. Retry with valid JSON.";
              else out = await executeTool(tc.name, tc.args || {});
              this.emit("toolResult", { name: tc.name, out });
              if (tc.name === "write_file" && typeof tc.args.path === "string") {
                const sp = safePath(tc.args.path);
                if (sp) { writtenFiles.add(sp); writtenByTools.push({ path: sp, bytes: String(tc.args.content || "").length, via: "tool" }); }
              }
              if (tc.name === "delete_file" && typeof tc.args.path === "string") {
                const sp = safePath(tc.args.path); if (sp) deletedFiles.add(sp);
              }
        ...
        if (signal.aborted) finalText = "Build stopped. Your project files have been preserved.";
        else if (finishMessage !== null && typeof finishMessage === "string" && finishMessage.trim()) finalText = finishMessage.trim();
        ...
        } else finalText = summarizeAgentAction(mode, Array.from(writtenFiles), Array.from(deletedFiles), userText);
        finalText = finalText.trim();
        if (finalText) S.addMessage({ role: "assistant", content: finalText, ts: Date.now(), mode });
        this.applyPluginInjections();
        this.emit("done", { mode, text: finalText, finishMessage });
        return { text: finalText };
      } catch (e) {
        if (e.name === "AbortError" || signal.aborted) { this.emit("done", { mode, text: "", aborted: true }); return { aborted: true }; }
        this.emit("error", e); throw e;
      } finally { this.running = false; this.emit("idle"); }
    },
    applyPluginInjections() {
      const S = window.Vecode.State;
      const enabledIds = window.Vecode.Plugins.PLUGINS.filter((p) => S.isPluginEnabled(p.id)).map((p) => p.id);
      const options = {}; for (const id of enabledIds) options[id] = S.getPlugin(id).options || {};
      const next = window.Vecode.Plugins.applyInjections(S.state.files, enabledIds, options);
      const changed = [];
      for (const p of Object.keys(next)) if (next[p] !== S.state.files[p]) changed.push(p);
      if (changed.length) { S.state.files = next; S.saveSoon(); this.emit("filesWritten", changed.map((p) => ({ path: p, via: "plugin" }))); }
    },
    async reviewPass() { if (this.running) return; return this.run("", "review"); }
  };
